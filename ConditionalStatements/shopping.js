function shopping(input) {
    let budget = Number(input[0]);
    let videoCards = Number(input[1]);
    let processor = Number(input[2]);
    let ram = Number(input[3]);

    let sumVideoCards = videoCards * 250;
    let priceProcessor = sumVideoCards * 0.35;
    let sumProcessor = priceProcessor * processor;
    let priceRam = sumVideoCards * 0.1;
    let sumRam = ram * priceRam;

    let total = sumProcessor + sumRam + sumVideoCards;
    if (videoCards > processor) {
        total = total * 0.85;
    }
    if (budget >= total) {
        console.log(`You have ${(budget - total).toFixed(2)} leva left!`);
    } else {
        console.log(`Not enough money! You need ${(total - budget).toFixed(2)} leva more!`);
    }

}

shopping(["900", "2", "1", "3"]);
